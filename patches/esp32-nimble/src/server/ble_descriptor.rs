...
impl BLEDescriptor {
  ...
  pub(super) extern "C" fn handle_gap_event(
    ...
    let descriptor_ptr: *mut _ = &mut descriptor;
    ...
    match ctxt.op as _ {
      esp_idf_sys::BLE_GATT_ACCESS_OP_READ_DSC => {
        ...
        unsafe {
          if (*(ctxt.om)).om_pkthdr_len > 8 || descriptor.value.len() <= (desc.mtu() - 3) as _ {
            if let Some(callback) = (&mut *descriptor_ptr).on_read.as_mut() {
              let descriptor_ref = &mut *descriptor_ptr;
              callback(&mut descriptor_ref.value, &desc);
            }
          }
        }
      ...
      esp_idf_sys::BLE_GATT_ACCESS_OP_WRITE_DSC => {
        ...
        unsafe {
          if let Some(callback) = (&mut *descriptor_ptr).on_write.as_mut() {
            let desc = crate::utilities::ble_gap_conn_find(conn_handle).unwrap();
            let mut arg = OnWriteDescriptorArgs {
              current_data: (&*descriptor_ptr).value.value(),
              recv_data: &buf,
              desc: &desc,
              reject: false,
              error_code: 0,
