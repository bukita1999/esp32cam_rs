...
impl BLECharacteristic {
  ...
  pub(super) extern "C" fn handle_gap_event(
    ...
    let characteristic_ptr: *mut _ = &mut characteristic;
    ...
    match ctxt.op as _ {
      esp_idf_sys::BLE_GATT_ACCESS_OP_READ_CHR => {
        ...
        unsafe {
          if (*(ctxt.om)).om_pkthdr_len > 8 || characteristic.value.len() <= (desc.mtu() - 3) as _ {
            if let Some(callback) = (&mut *characteristic_ptr).on_read.as_mut() {
              let characteristic_ref = &mut *characteristic_ptr;
              callback(&mut characteristic_ref.value, &desc);
            }
          }
        }
      ...
      esp_idf_sys::BLE_GATT_ACCESS_OP_WRITE_CHR => {
        ...
        unsafe {
          if let Some(callback) = (&mut *characteristic_ptr).on_write.as_mut() {
            let desc = crate::utilities::ble_gap_conn_find(conn_handle).unwrap();
            let mut arg = OnWriteArgs {
              current_data: (&*characteristic_ptr).value.value(),
              recv_data: &buf,
              desc: &desc,
              reject: false,
              error_code: 0,
